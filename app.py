...
def send_welcome(message):
    ...
    welcome_text = (
        "👋 Привет, я Бот, созданный @karimowda для помощи пользователям в поиске информации.\n\n"
        "Что вас интересует?"
    )
...
def handle_message(message):
    try:
        ...
        if user_input == "ℹ️ О боте":
            bot.send_message(message.chat.id, "Я использую GPT для поиска и генерации ответов. Напиши вопрос!")
            return
        elif user_input == "📞 Связаться":
            bot.send_message(message.chat.id, "Связаться с автором: @karimowda")
            return
